...
export default function Dashboard() {
  ...
  // Active Tab State
  const [activeTab, setActiveTab] = useState<'video' | 'properties' | 'owner' | 'bookings'>('bookings');
  ...
  const loadDashboardData = async () => {
    setIsLoading(true);
    try {
      const [v, o, p, b] = await Promise.all([
        getHomeVideo(),
        getOwnerPhoto(),
        getProperties(),
        getBookings(),
    ...
    <div className="min-h-screen bg-transparent text-white pt-28 pb-20">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
          ...
          {/* Stat 2 */}
          <div className="bg-white/[0.02] backdrop-blur-md border border-white/10 p-6 rounded-sm">
            <span className="text-[10px] uppercase tracking-widest text-gray-500 font-semibold block mb-2">Private Tour Bookings</span>
            <div className="flex items-baseline gap-2">
              <span className="font-serif text-3xl text-white font-light">{stats.totalBookings}</span>
              <span className="text-[10px] text-gold font-mono">Inquiries</span>
            </div>
          </div>
          {/* Stat 3 */}
          <div className="bg-white/[0.02] backdrop-blur-md border border-white/10 p-6 rounded-sm">
                   ...
                   >
                     ...
                     {filteredBookings.length === 0 ? (
                       <div className="text-center py-20 border border-dashed border-white/10 rounded">
                         <span className="text-gray-500 text-xs uppercase tracking-widest">No matching bookings recorded</span>
                       </div>
                     ) : (
                       <div className="space-y-4">
                         {filteredBookings.map((b) => (
                           <div
                             key={b.id}
                             className="bg-white/[0.02] backdrop-blur-sm border border-white/10 p-6 rounded-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:border-gold/25 transition-all"
                           >
                             <div className="space-y-2">
                               <div className="flex items-center gap-2">
                                 <span className="font-serif text-lg text-white font-medium">{b.fullName}</span>
                                 <span className="text-[9px] uppercase tracking-widest font-mono bg-slate-900 border border-white/10 px-2 py-0.5 rounded text-gold">
                                   {b.propertyName}
                                 </span>
                               </div>
 
                               <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1 text-xs text-gray-400">
                                 <div><span className="text-gray-500">WhatsApp:</span> {b.whatsappNumber}</div>
                                 <div><span className="text-gray-500">Phone:</span> {b.phoneNumber}</div>
                                 <div><span className="text-gray-500">Preferred:</span> {b.preferredDate} at {b.preferredTime}</div>
                                 <div><span className="text-gray-500">Logged:</span> {new Date(b.dateAdded).toLocaleDateString()}</div>
                               </div>
 
                               {b.additionalNotes && (
                                 <div className="text-xs text-gray-500 font-light border-l border-white/10 pl-3 mt-2 max-w-xl">
                                   "{b.additionalNotes}"
                                 </div>
                               )}
                             </div>
                             ...
                             <button
                               ...
                               className="bg-red-950/10 hover:bg-red-950/40 border border-red-500/15 hover:border-red-500/30 text-red-400 p-2.5 rounded-sm transition-all self-end md:self-center cursor-pointer"
